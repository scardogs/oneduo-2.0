...
'Intent confidence derived from Passive Emphasis Reconstructor â€“ anchors reasoning to observed human emphasis';
...
BEGIN
  ...
  IF NEW.status = 'finalized' AND (OLD.status IS NULL OR OLD.status != 'finalized') THEN
    IF NOT public.can_finalize_artifact(NEW.id) THEN
      RAISE EXCEPTION 'EXECUTION IMPOSSIBLE UNTIL GOVERNANCE = TRUE: Cannot finalize artifact â€“ pending reasoning entries require human decision';
    END IF;
  END IF;
  RETURN NEW;
END;
